...
class SentimentDataset(BaseDataset):
    """docstring for SentimentDataset"""
    @staticmethod
    ...
    def __init__(self, opt):
        super(SentimentDataset, self).__init__(opt)
        ...
        self.CNFND_LABELS = ['beauty_salon', 'nursery', 'delicatessen', 'volcano', 'slum',
                        'wheat_field', 'hospital_room', 'veterinarians_office', 'indoor', 'outdoor']
    ...
    def _parse_emo_by_path(self, img_path):
        ...
        if img_emo not in self.EMO_LABELS:
            raise Exception("The emotion is not defined, info: [%s, %s]" % (img_path, img_emo))
        return self.EMO_LABELS.index(img_emo)

    def _parse_cnfnd_by_path(self, img_path):
        img_key = self._parse_key_by_path(img_path)
        img_ds = self.cnfnd_dict[img_key]
        if img_ds not in self.CNFND_LABELS:
            raise Exception("The confounder is not defined, info: [%s, %s]" % (img_path, img_ds))
        return self.CNFND_LABELS.index(img_ds)
